...
if __name__ == "__main__":
    ...
    # init yaw
    last_vel_z = 0
    lemniscate = Lemniscate(scale=2,c=0.5)
    hover_time = 10
    yaw= 0
    # 指定保存的CSV文件名
    ...
    while True:
        ...
        if(current_time-poly_start_time<hover_time):
            # just hover
            exp_pos = np.array([0,0,2.33])
            exp_vel = np.array([0,0,0])
            yaw = np.pi
        elif(current_time-poly_start_time-hover_time < 20*np.pi ):
            ...
            exp_vel = lemniscate.update_de(torch.tensor([t])).numpy()
            exp_vel = exp_vel[0]
